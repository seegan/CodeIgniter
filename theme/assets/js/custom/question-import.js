jQuery(document).ready(function() { 
	...
	function analyzeUploadedFile(upload_id = 0) {
        jQuery.ajax({
            type: "POST", 
            url: import_ajaxurl, 
            data: { action: 'analyzeUploadedFile', import_type: 'question', upload_id: upload_id, }, 
            dataType: "json",
            success: function (data) {
            	var ul_txt = '<ul>';
            	if(data.success == true) {
            		ul_txt += '<li>Valid Data (Estimated) : '+data.total_count+'</li>';
	...
	function importAccept(update_id = 0, import_type) {
        jQuery.ajax({ 
            ...
            success: function (data) {
            	if(data.success == true) {
            		window.location.href = site_url+'admin/question/import/'+data.upload_id;
            	}
            }
        });
	}
	...
	function importIntervel(){
	  	...
	  	window.tid = setInterval(function() {
			...
			var select_list = jQuery('tr.list-import.deactive:lt(1)').map(function() {
			    return jQuery(this).attr('data-baseid');
			}).get().join();
		    jQuery('tr.list-import.deactive:lt(1)').removeClass('deactive').addClass('de-activate');
	...
	function createImportData(select_list) {
		if(select_list) {
		    jQuery.ajax({
		        type: "POST", 
		        url: import_ajaxurl, 
		        data: { action: 'createQuestionFromImport', select_list: select_list, import_id: jQuery('#import_id').val() }, 
		        dataType: "json",
		        success: function (data) {
